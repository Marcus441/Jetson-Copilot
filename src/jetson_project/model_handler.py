...
class ModelHandler:
    ...
    def load_model(self):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(
            self.model_name,
            torch_dtype=torch.float16 if self.device == "cuda" else torch.float32, # Use float16 for GPU for performance, bfloat16 is not always supported on Jetson
    ...
    def generate_completion(self, prompt, max_new_tokens=100):
        ...
        messages = [
            {"role": "system", "content": "You are Qwen, created by Alibaba Cloud. You are a helpful code completion assistant."},
